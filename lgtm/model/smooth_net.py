...
class SmoothNetResBlock(nn.Module):
    ...
    Shape:
        ...
        Output: (*, in_channels)
    """
    def __init__(self, in_channels: int, hidden_channels: int, dropout=0.5):
        super().__init__()
...
class SmoothNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        assert T == self.window_size, ('Input sequence length must be equal to the window size. ', f'Got x.shape[-1]=={T} and window_size=={self.window_size}')
